...
export class EntityMove {
    ...
    public update(tween: number = 0.2): void {
        ...
        if (this._node.isCurrentPlayer && this._node.cameraController) {
            // Use the new public method to rotate the camera
            this._node.cameraController.rotateCameraRoot(-this._node.rotation.y + this._game.deltaCamY);
        }
    }
