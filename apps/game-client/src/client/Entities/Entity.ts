...
export class Entity extends TransformNode {
    ...
    public update(delta): any {
        // Skip if mesh is not loaded yet
        if (!this.mesh) return;
        ...
        // Update camera if we're the current player
        if (this.isCurrentPlayer && this.cameraController) {
            try {
    ...
    /**
     * Safely access camera controller
     */
    ...
    /**
     * Safely set camera controller
     */
    public setCameraController(camera: PlayerCamera) {
        this.cameraController = camera;
        console.log(`Camera controller set for entity ${this.sessionId}`);
        return this;
    }
}